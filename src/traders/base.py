...
class BaseTrader(ABC):
    def __init__(self, trader_id: int, config: dict, trader_type: str):
        ...
        self.config = config
        self.fundarmental_price = config['fundamental_price']
        ...
        self.cash = self.fundarmental_price * self.stock

        # 策略权重参数（对数正态分布，三者共享sigma，mean分别可设）
        mu1 = config[trader_type]["fundamental_mu"]
        mu2 = config[trader_type]["chartist_mu"]
        mu3 = config[trader_type]["noise_mu"]
        sigma = config["strategy_sigma"]
        self.g1 = np.random.lognormal(mean=mu1, sigma=sigma)
        self.g2 = np.random.lognormal(mean=mu2, sigma=sigma)
        self.n = np.random.lognormal(mean=mu3, sigma=sigma)
        ...
        self.emotion_weight = np.random.lognormal(mean=config.get("emotion_weight_mu", 1.0), sigma=sigma)
    ...
    def __repr__(self):
        return (f"<Trader {self.trader_id} ({self.type}) | "
                ...
                f"g1={self.g1:.2f}, g2={self.g2:.2f}, n={self.n:.2f}, tau={self.tau:.2f}> "
                )
